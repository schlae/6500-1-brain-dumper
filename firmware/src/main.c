...
static inline __attribute__((always_inline)) void send_data(uint8_t data) {
  ...
  while(!(TIFR2 & _BV(OCF2A))); // went high
  TIFR2 |= _BV(OCF2A);
  while(!(TIFR2 & _BV(OCF2A))); // went lo
  TIFR2 |= _BV(OCF2A);
  while(!(TIFR2 & _BV(OCF2A))); // went hi
  TIFR2 |= _BV(OCF2A);
  while(!(TIFR2 & _BV(OCF2A))); // went lo
  TIFR2 |= _BV(OCF2A);
}
...
int main(void) {
  ...
  UBRR0H = CALC_BPS(UART0_BAUDRATE) >> 8;
  UBRR0L = CALC_BPS(UART0_BAUDRATE) & 0xff;
  UCSR0B = (_BV(TXEN0));
  uint8_t code[] = { 0xA9 ,0x00       // lda #0
                    ,0x85 ,0x82       // sta $82
                    ,0xA2 ,0x00       // ldx #0
                    ,0xBD ,0x00 ,0x08 //lda $800,x
                    ,0x85 ,0x80       // sta $80 store data to PORTA
                    ,0xA9 ,0xFF       // lda #$ff PORTC=ff
                    ,0x85 ,0x82       // sta $82
                    ,0xA9 ,0x00       // lda #00 PORTC=00
                    ,0x85 ,0x82       // sta $82
                    ,0xE8             // inx
                    ,0xD0 ,0xF0       // bne loop
                    ,0xe6 ,0x08       // inc $08
                    ,0xa5, 0x08       // lda $08 modify start address
                    ,0xc9, 0x10       // cmp $10
                    ,0xd0 ,0xE6       // bne loop2
  ...
  TCCR2B = _BV(CS20); // /16MHz, CTC mode
